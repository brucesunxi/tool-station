...
const platforms = [
  ...
  {
    id: 'email', name: 'Email',
    icon: <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
    bg: 'hover:bg-gray-100 dark:hover:bg-gray-700',
...
export default function ShareButton({ title, url, description }: ShareButtonProps) {
  ...
  const handleClick = () => {
    ...
    if (typeof navigator !== 'undefined' && navigator.share && window.innerWidth < 1024) {
      navigator.share({ title, text: description || title, url: shareUrl }).catch(() => setOpen(true))
      return
    }
    setOpen(!open)
  }
  ...
  const shareTo = (p: (typeof platforms)[0]) => {
    if (p.id === 'copy') { copyLink(); return }
    const encodedUrl = encodeURIComponent(shareUrl)
    const encodedTitle = encodeURIComponent(title)
    const encodedDesc = description ? encodeURIComponent(description) : undefined
    const wu = p.url(encodedUrl, encodedTitle, encodedDesc)
    if (wu) window.open(wu, '_blank', 'noopener')
    setOpen(false)
  }
            ...
            <div className="space-y-0.5">
              {platforms.map((p) => (
                <button key={p.id} onClick={() => shareTo(p)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm rounded-xl transition-colors ${p.bg}`}>
                  <span className="shrink-0 w-5 flex justify-center">{p.icon}</span>
                  <span>{p.id === 'copy' ? (copied ? 'Copied! ✓' : p.name) : p.name}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex items-end">
          ...
          <div ref={panelRef} className="relative w-full bg-white dark:bg-gray-800 rounded-t-2xl shadow-2xl p-5 pb-8">
            ...
            <div className="grid grid-cols-4 gap-3">
              {platforms.map((p) => (
                <button key={p.id} onClick={() => shareTo(p)}
                  className={`flex flex-col items-center gap-1.5 p-3 rounded-xl transition-colors ${p.bg}`}>
                  <span className="flex items-center justify-center w-8 h-8">{p.icon}</span>
                  <span className="text-[10px] text-gray-500 dark:text-gray-400 truncate w-full text-center leading-tight">
                    {p.id === 'copy' ? (copied ? 'Copied!' : 'Copy') : p.name}
                  </span>
                </button>
              ))}
            </div>
