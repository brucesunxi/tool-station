...
export default function LearningPage() {
  const t = useTranslations('tools.ai-learning')
      ...
      <div className="bg-white dark:bg-gray-800 border rounded-xl p-6 mb-8">
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          <select value={level} onChange={e => setLevel(e.target.value)} className="px-3 py-2 border rounded-lg text-sm dark:bg-gray-700 dark:border-gray-600">
            <option value="">{ct("anyLevel")}</option>
            <option value="beginner">{ct("levelBeginner")}</option>
            <option value="intermediate">{ct("levelIntermediate")}</option>
            <option value="advanced">{ct("levelAdvanced")}</option>
          </select>
        </div>
