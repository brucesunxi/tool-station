...
export default function PoemPage() {
  const t = useTranslations('tools.ai-poem')
      ...
      <div className="bg-white dark:bg-gray-800 border rounded-xl p-6 mb-8">
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          <select value={style} onChange={e => setStyle(e.target.value)} className="px-3 py-2 border rounded-lg text-sm dark:bg-gray-700 dark:border-gray-600">
            <option value="">{ct("anyStyle")}</option>
            <option value="sonnet">{ct("styleSonnet")}</option>
            <option value="haiku">{ct("styleHaiku")}</option>
            <option value="free verse">{ct("styleFreeVerse")}</option>
            <option value="limerick">{ct("styleLimerick")}</option>
            <option value="acrostic">{ct("styleAcrostic")}</option>
          </select>
        </div>
