...
export default function LoveLetterPage() {
  const t = useTranslations('tools.ai-love-letter')
      ...
      <div className="bg-white dark:bg-gray-800 border rounded-xl p-6 mb-8">
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          <select value={tone} onChange={e => setTone(e.target.value)} className="px-3 py-2 border rounded-lg text-sm dark:bg-gray-700 dark:border-gray-600">
            ...
            <option value="romantic">{ct("toneRomantic")}</option>
            <option value="poetic">{ct("tonePoetic")}</option>
            <option value="heartfelt">{ct("toneHeartfelt")}</option>
            <option value="playful">{ct("tonePlayful")}</option>
          </select>
        </div>
