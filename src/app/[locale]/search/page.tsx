...
export default function SearchPage() {
  ...
  const cats = useMemo(() => {
    ...
    const labels: Record<string, string> = { all: 'All Tools', image: 'Image', pdf: 'PDF', ai: 'AI', dev: 'Dev', text: 'Text', color: 'Color' }
    return keys.map(k => ({ key: k, label: labels[k] || k }))
  }, [])

  const results = useMemo(() => {
    if (!query.trim() && category === 'all') return tools
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500">Find the tool you need among {tools.length} free online tools.</p>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input type="text" value={query} onChange={e => setQuery(e.target.value)}
          placeholder="Search by tool name, description, or category..."
          autoFocus
            ...
            <Link key={tool.href} href={tool.href}
              ...
              <div className="min-w-0 flex-1">
                <p className="font-medium text-sm truncate">
                  {tool.title}
                  {tool.isNew && <span className="ml-1.5 text-[10px] bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-300 px-1.5 py-0.5 rounded font-medium">NEW</span>}
                </p>
      ...
      {query && <p className="mt-4 text-xs text-gray-400 text-right">{results.length} of {tools.length} tools</p>}

      <div className="mt-12 pt-8 border-t">
        <h2 className="text-xl font-bold mb-4">Free Online Tools</h2>
        <p className="text-sm text-gray-500">
          Browse all {tools.length} free online tools across 6 categories: Image, PDF, AI, Developer, Text, and Color.
